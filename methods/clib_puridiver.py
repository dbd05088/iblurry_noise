...
class CL_PURI(ER):
    ...
    def get_cls_cos_score(self, df):
        ...
        weights = self.model.fc.weight.data.detach().cpu()
    ...
    def puridiver_split(self, epoch, dataset, n, plot_gmm=False):
        ...
        loader = DataLoader(dataset,
                            shuffle=True,
                            batch_size=min(len(self.memory.images),64),
                            num_workers=2,
                            )
        losses = torch.tensor([])
    ...
    def online_train(self, sample, batch_size, n_worker, iterations=1, stream_batch_size=0):
        ...
        for i in range(iterations):
            ...
            if len(self.memory) > 0 and batch_size - stream_batch_size > 0:
                ...
                if train_loss != 0:
                    self.coeff = self.origin_coeff / train_loss
                if self.coeff > self.origin_coeff:
                    self.coeff = self.origin_coeff
                #coeff += self.coeff
                #logger.info(f"[{batch_idx}/{len(dataloader)}] loss: {l}, diversity coeff: {self.coeff}, "f"{1000 * (time.time() - start)} msec")
