...
class ER:
    ...
    def online_evaluate(self, test_list, sample_num, batch_size, n_worker):
        ...
        exp_test_df = test_df[test_df['label'].isin(self.exposed_classes)]
        ...
        print("exposed_class", len(self.exposed_classes), "test_dataset 길이", len(test_dataset))
    ...
    def evaluation(self, test_loader, criterion):
        ...
        with torch.no_grad():
            for i, data in enumerate(test_loader):
                ...
                x = x.to(self.device)
                #print("before y", y)
                y = torch.Tensor([self.memory.cls_dict[label] for label in y.tolist()]).long()
                #print("after y", y)
                y = y.to(self.device)
                logit = self.model(x)
        ...
        print("total_correct", total_correct, "total_num_data", total_num_data)
