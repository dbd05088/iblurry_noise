...
def main():
    ...
    method = select_method(
        args, criterion, device, train_transform, test_transform, n_classes, weak_transform
    )
    ...
    samples_cnt = 0
    test_datalist = get_test_datalist(args, args.rnd_seed)
    ...
    for cur_iter in range(args.n_tasks):
        ...
        # get datalist
        cur_train_datalist = get_train_datalist(args, cur_iter, args.rnd_seed)
        ...
        print(len(cur_train_datalist))
        df = pd.DataFrame(cur_train_datalist)
        print("labels", list(set(df['label'].values)))
        print("cur_iter", cur_iter, "train_len", len(cur_train_datalist))
        #print(df)
        ...
        df = pd.DataFrame(method.memory.datalist)
        print(df.label.value_counts())
        print(df.klass.value_counts())
        ...
        print("TaskAcc", task_acc)
        print("Memory clean ratio", n_clean / method.memory_size)
