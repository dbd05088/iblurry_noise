...
class ImageDataset(Dataset):
    def __init__(self, data_frame: pd.DataFrame, dataset: str, dataset_path=None, transform=None, cls_list=None, data_dir=None,
                 preload=False, device=None, transform_on_gpu=False, origin_list=None, test_list=None, prob=None):
        self.data_frame = data_frame
        ...
        if self.preload:
            ...
            for idx in range(len(self.data_frame)):
                ...
                # 추가
                if self.origin_list is not None:
                    sample["test_img"] = self.test_list[idx]
                    sample["origin_img"] = self.origin_list[idx]
                if self.prob is not None:
                    sample['prob'] = self.prob[idx, :]
    ...
    def __getitem__(self, idx):
        if self.preload:
            #print("loaded len", len(self.loaded_images), "idx", idx)
            #return list(np.array(self.loaded_images)[idx])
    ...
    def get_data_gpu(self, indices):
        ...
        for i in indices:
            ...
            images.append(self.loaded_images[i]["image"])
...
class MemoryDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.robust_transform is not None:
            origin = self.robust_transform[0](image)
            sample["origin_img"] = origin
            sample["test_img"] = self.robust_transform[2](image)
            image = self.robust_transform[1](image)
